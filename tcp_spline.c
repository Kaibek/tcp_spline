...
static void stable_rtt_bw(struct sock *sk)
{
    ...
    if (scc->fairness_rat >= 4 || ((u64)scc->bytes_in_flight << 1) < scc->curr_cwnd) {
...
static u32 fairness_check(struct sock *sk, u32 cwnd)
{
    ...
    {
        u64 cwnd_second = (((u64)cwnd * 15) >> SCALE_BW_RTT);
        cwnd = (u32)(cwnd_second + scc->fairness_rat << 7);
...
static void spline_max_cwnd(struct sock *sk)
{
    ...
    tmp = (u64)scc->fairness_rat * (scc->bw / (scc->throughput << 2) + scc->last_acked_sacked);
...
static void drain_probe(struct sock *sk)
{
    ...
    if (scc->last_ack < scc->curr_ack || is_loss(sk) || scc->throughput << 4 < scc->bw)
        scc->curr_cwnd = (u32)((((u64)scc->curr_cwnd + scc->last_acked_sacked) * 12) >> SCALE_BW_RTT);
    else
